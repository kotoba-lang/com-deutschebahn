(ns deutschebahn.main
  "Kotodama WASM entrypoint for the Deutschebahn clean-room actor (L4) — Clojure port.
  ...
  facts (`deutschebahn.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "deutschebahn")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"deutschebahn.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "deutschebahn-compat" :tier tier :entities entities} 200])
...
(defn start! [] :deutschebahn-compat/ready)
